...
class VerseLastWordGenerator(BaseQuestionGenerator):
    ...
    def get_question_title(self, verse, words_list, translation):
        question_title = f' متن سوال : انتهای این آیه با چه کلمه‌ای تمام می‌شود؟ سوره: {verse.surah.name} آیه {verse.verse_number} '
    ...
    def get_wrong_options(self, verse, correct_option, valid_verses, words_list):
        ...
        for v, words, _ in nearby_verses:
            word = words[-1]
            if word['clean'] != words_list[-1]['clean'] and word['clean'] not in wrong_options['clean']:
        ...
        if len(wrong_options['arabic']) < 3:
            ...
            extra_words = self.extra_words_provider.get_extra_words(
                ...
                need_count=need_count, reverse_ordering=True
            )
            for arabic, clean in extra_words:
                if clean not in wrong_options['clean'] and clean != words_list[-1]['clean']:
