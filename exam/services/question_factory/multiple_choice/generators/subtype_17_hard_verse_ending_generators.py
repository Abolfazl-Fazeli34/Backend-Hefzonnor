...
class VerseLastWordHardGenerator(BaseQuestionGenerator):
    def __init__(self, hard_end_provider):
        self.hard_end_provider = hard_end_provider()

    def get_question_title(self, verse, words_list, translation):
        return f' متن سوال : انتهای این آیه با چه کلمه ای تمام می‌شود؟ سوره: {verse.surah.name} آیه {verse.verse_number} ' + ' '.join(
            word['arabic'] for word in words_list[:-1])

    def get_correct_options(self, verse, words_list, translation):
        return words_list[-1]['arabic']

    def get_wrong_options(self, verse, correct_option, valid_verses, words_list):
        wrong_options = {'arabic': [], 'clean': []}
        second_last_word = words_list[-2]['clean'] if len(words_list) > 1 else ''
        third_last_word = words_list[-3]['clean'] if len(words_list) > 2 else ''

        hard_options = self.hard_end_provider.get_similar_last_words(
            verse_id=verse.id,
            correct_word=words_list[-1]['clean'],
            second_last_word=second_last_word,
            third_last_word=third_last_word,
            need_count=3
        )
        ...
        if len(wrong_options['arabic']) < 3:
            fallback_options = self.get_wrong_options_with_nearby_fallback(verse, words_list[-1]['clean'], valid_verses,
